  ...
  class XMLPRTIO
    ...
    def save
      ...
      @tbls.each_primary do |value| # save all tables
        xml.add_element(value.name,
          {"type" => "PrimaryTable",
           "addr" => value.table.location.to_s(16),
           "size" => value.table.count.to_s,
           "base" => value.table.type.to_s})

        tblfile = File.new @path + "_#{value.name}.xml", "w"
        tbldoc = REXML::Document.new
        tblxml = tbldoc.add_element("Table")
        do_save(tblxml, value.table)
        tbldoc.write tblfile, 2
        tblfile.close
      end

      @tbls.each_single do |cell|
        next if cell.ref_cnt == 1
        xml.add_element(cell.name,
          {"type" => "MultiRefedTable",
           "addr" => cell.table.location.to_s(16),
           "size" => cell.table.count.to_s,
           "base" => cell.table.type.to_s})

        tblfile = File.new @path + "_#{cell.name}.xml", "w"
        tbldoc = REXML::Document.new
        tblxml = tbldoc.add_element("Table")
        do_save(tblxml, cell.table)
        tbldoc.write tblfile, 2
        tblfile.close
      end
    ...
    def load(strict)
      doc = REXML::Document.new(File.new(@path + "_idx.xml", "r"))
      ...
      root.each_element_with_attribute("type", "MultiRefedTable") do |ele| # load for tables
        ...
        tblfile = File.new(@path + "_#{ele.name}.xml")
        tblxml = REXML::Document.new(tblfile)
        tblroot = tblxml.root
        do_tblload(tblroot, tbl)
        tblxml.close
      end
